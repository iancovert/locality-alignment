...
group.add_argument("--student-pretrained", action="store_true", default=False, help="Start with pretrained version of specified network (if avail)")
...
group.add_argument("--teacher-model", default="vit_tiny_patch16_224.augreg_in21k_ft_in1k", type=str, metavar="MODEL", help='Name of model to distill from (default: "vit_base_patch16_224.augreg_in21k_ft_in1k")')
group.add_argument("--teacher-pretrained", action="store_true", default=False, help="Start with pretrained version of specified network (if avail)")
...
def main():
    ...
    if args.teacher_checkpoint:
        ...
        if utils.is_primary(args):
            _logger.info(f"Loaded teacher model from {args.teacher_checkpoint}, incompatible keys: {non_matching_keys}")
    teacher_patch_size = teacher.patch_embed.patch_size[0]
    teacher = convert_to_mim_teacher_model(teacher)
    teacher.eval()
    ...
    student = convert_to_mim_student_model(
        encoder,
    ...
    if utils.is_primary(args):
        _logger.info("Teacher model summary")
        _logger.info(
            summary(
                teacher,
                input_size=(args.batch_size,) + tuple(data_config["input_size"]),
                verbose=0,
            )
        )
        _logger.info("Student model summary")
        _logger.info(
            summary(
                student,
                input_size=[
                    (args.batch_size,) + tuple(data_config["input_size"]),
                    (args.batch_size, num_patches),
                ],
                verbose=0,
            )
        )
    ...
    if args.loss_function == "mse":
        ...
        if utils.is_primary(args):
            _logger.info(f"Baseline loss: {baseline_metrics['baseline_loss']:.5f}")

        # Apply normalization.
        scaling_factor = 1 / (baseline_metrics["baseline_loss"] ** 0.5)
        if utils.is_primary(args):
            _logger.info(f"Scaling teacher predictions by factor = {scaling_factor:.2f}")
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        def _forward():
            with amp_autocast():
                ...
                with torch.no_grad():
                    preds = teacher(input)
                    preds_repeat = preds.repeat_interleave(mask_ratio, 0)

                # Student reconstructions.
                input_repeat = input.repeat_interleave(mask_ratio, 0)
                reconstruction = student(input_repeat, mask)
                if loss_function == "mse":
                    loss = torch.nn.functional.mse_loss(reconstruction, preds_repeat)
                elif loss_function == "cosine":
                    loss = 1 - torch.nn.functional.cosine_similarity(reconstruction, preds_repeat, dim=-1).mean()
                elif loss_function == "l1":
                    loss = torch.nn.functional.l1_loss(reconstruction, preds_repeat)
                    ...
                    loss = torch.mean((1 - mask).unsqueeze(-1) * (reconstruction - preds_repeat) ** 2)
                elif loss_function == "unmasked_mse":
                    # Calculate loss for unmasked tokens only.
                    if reconstruction.shape[1] > mask.shape[1]:
                        num_prefix_tokens = reconstruction.shape[1] - mask.shape[1]
                        mask = torch.nn.functional.pad(mask, (num_prefix_tokens, 0), value=1)
                    loss = torch.mean(mask.unsqueeze(-1) * (reconstruction - preds_repeat) ** 2)
