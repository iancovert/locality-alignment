...
class dBOTCLIPVisionTransformer(nn.Module):
...
def _create_dbot_vit(variant: str, pretrained: bool = False, **kwargs) -> dBOTCLIPVisionTransformer:
    kwargs["pretrained_cfg"] = _cfg()

    model = timm.models._builder.build_model_with_cfg(
        dBOTCLIPVisionTransformer,
        variant,
        pretrained=False,
        **kwargs,
    )
...
@register_model
def vit_base_patch16_dbot_clip_224(pretrained: bool = False, **kwargs) -> dBOTCLIPVisionTransformer:
    model_args = dict(
        patch_size=16,
        embed_dim=768,
        depth=12,
