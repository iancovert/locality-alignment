...
def _create_dbot_vit(variant: str, pretrained: bool = False, **kwargs) -> dBOTRandomVisionTransformer:
    ...
    if pretrained and variant.startswith("vit_base"):
        ...
        local_path = os.path.join(cache_dir, "checkpoints", "84.5_dbot_base_pre.pth")

        url = "https://lf3-nlp-opensource.bytetos.com/obj/nlp-opensource/mmodal/dbot/84.5_dbot_base_pre.pth"
        if os.path.exists(local_path):
            logging.info(f"Loading 84.5_dbot_base_pre.pth from local path: {local_path}")
            checkpoint = torch.load(local_path, map_location=torch.device("cpu"))
        else:
            logging.info(f"Downloading 84.5_dbot_base_pre.pth from url: {url}")
            checkpoint = torch.hub.load_state_dict_from_url(url, map_location=torch.device("cpu"))

        # Model loading logic from https://github.com/liuxingbin/dbot/blob/main/main_finetune.py
